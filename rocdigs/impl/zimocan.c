  ...
  /* Program command. */
    ...
    if( slot != NULL ) {
      if( wProgram.getcmd( node ) == wProgram.get ) {
        byte* msg = allocMem(32);
        msg[0] = __makePacket(msg+1, TRACK_CONFIG_GROUP, TRACK_CONFIG_READ, MODE_CMD, 8, data->NID, slot->nid,
            cv&0xFF, (cv&0xFF00)>>8, (cv&0xFF0000)>>16, (cv&0xFF000000)>>24, 0, 0);
        ...
        byte* msg = allocMem(32);
        msg[0] = __makePacket(msg+1, TRACK_CONFIG_GROUP, TRACK_CONFIG_WRITE, MODE_CMD, 8, data->NID, slot->nid,
            cv&0xFF, (cv&0xFF00)>>8, (cv&0xFF0000)>>16, (cv&0xFF000000)>>24, val, 0);
...
static int __makeStringPacket( byte* msg, int group, int cmd, int mode, int nid, int nidTarget, int value1, int value2, const char* str ) {
  ...
  msg[11]  = ((value1 >> 16) & 0xFF);
  msg[12]  = ((value1 >> 24) & 0xFF);

  msg[13]  = (value2 & 0xFF);
  msg[14]  = ((value2 >> 8) & 0xFF);
  msg[15]  = ((value2 >> 16) & 0xFF);
  msg[16]  = ((value2 >> 24) & 0xFF);
