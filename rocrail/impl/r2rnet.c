...
static void __handleNetReq(iOR2Rnet inst, iONode req) {
    ...
    StrOp.equals( wNetReq.getremoteid(req), wR2RnetIni.getid(data->props) ) )
  {
    ...
    if( block != NULL ) {
      ...
      Boolean reserved = block->lock(block, wLoc.getid(lc), wNetReq.getlocalbk(req), wNetReq.getrouteid(req), False, False, False, 0, NULL);
      ...
      if( route != NULL ) {
        ...
        if( reserved ) {
          ...
          block->lock(block, LocOp.getId(loc), wNetReq.getlocalbk(req), wNetReq.getrouteid(req), False, False, False, 0, NULL);
