...
static Boolean _cmd( iIBlockBase inst ,iONode cmd ) {
  ...
  if( command == NULL && state != NULL ) {
    ...
    ModelOp.setBlockOccupancy( AppOp.getModel(), wSelTab.getid(data->props), data->lockedId, StrOp.equals( wBlock.closed, state ), 0, 0, NULL );
...
static Boolean _lock( iIBlockBase inst, const char* id, const char* blockid, const char* routeid, Boolean crossing, Boolean reset, Boolean reverse, int indelay, const char* masterId ) {
  ...
  if( !StrOp.startsWith(blockid, wRoute.routelock) && wSelTab.ismanager(data->props) ) {
    ...
    if( block == NULL || !block->lock(block, id, blockid, NULL, crossing, reset, reverse, 0, NULL ) ) {
...
static void _modify( struct OSelTab* inst ,iONode props ) {
  ...
  for( i = 0; i < cnt; i++ ) {
    ...
    if( StrOp.equals("id", name) && StrOp.equals( value, wSelTab.getid(data->props) ) )
