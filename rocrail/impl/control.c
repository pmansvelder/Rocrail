  ...
  if( StrOp.equals( wBooster.name(), nodeName ) ) {
    ...
    /* ArchiveBox */
      ...
      if( abox != NULL ) {
        ...
        list = abox->find((obj)abox, wDataReq.gettext(nodeA), wDataReq.isfindintext(nodeA), wDataReq.isfindincategory(nodeA),
            wDataReq.isfindinfilename(nodeA), wDataReq.isfindindate(nodeA), wDataReq.getfromdate(nodeA), wDataReq.gettodate(nodeA));
        if( list != NULL ) {
          int i = 0;
...
static void _halt( iOControl inst ) {
  if( inst != NULL ) {
    ...
    TraceOp.trc( name, TRCLEVEL_INFO, __LINE__, 9999, "Informing interfaces..." );
    {
      ...
      while( di != NULL ) {
        di->halt((obj)di, wRocRail.ispoweroffonexit(AppOp.getIni()), wRocRail.isshutdownonexit(AppOp.getIni()) );
